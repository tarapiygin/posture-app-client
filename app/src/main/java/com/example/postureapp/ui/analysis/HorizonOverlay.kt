...
import com.example.postureapp.core.designsystem.AccentBlue
...
fun HorizonOverlay(
    ...
    val rollAligned = abs(roll) <= AnalysisViewModel.ROLL_TOLERANCE
    val pitchAligned = abs(pitch) <= AnalysisViewModel.PITCH_TOLERANCE
    val horizonColor = if (aligned) SuccessGreen else AccentBlue
    val hintMessages = buildList {
        if (!rollAligned) {
            add(
                if (roll > AnalysisViewModel.ROLL_TOLERANCE) {
        ...
        if (!pitchAligned) {
            add(
                if (pitch > AnalysisViewModel.PITCH_TOLERANCE) {
